...
// HECC mode also enables time-stamping feature
	...
	ECanaShadow.CANMC.bit.SCB = 1;                  //工作在eCAN模式
...
/* Disable all Mailboxes */
    ...
    // field) a shadow register is not required.
 	...
 	ECanaMboxes.MBOX0.MSGID.all = 0x11110000;       //设置发送邮箱的ID号，扩展帧
 	ECanaShadow.CANMD.all = ECanaRegs.CANMD.all;    //邮箱0为TX
 	ECanaShadow.CANMD.bit.MD0 = 0;
    ECanaRegs.CANMD.all = ECanaShadow.CANMD.all;
    ECanaMboxes.MBOX0.MSGCTRL.bit.DLC = 8;          //数据长度8字节
    ECanaMboxes.MBOX0.MSGCTRL.bit.TPL = 0;          //设置发送优先级
    ECanaMboxes.MBOX0.MSGCTRL.bit.RTR = 0;          //没有远方应答帧被请求
    ECanaMboxes.MBOX0.MDL.all = 0x01234567;         //向邮箱RAM区写数据
    ECanaMboxes.MBOX0.MDH.all = 0x89ABCDEF;         
    ECanaShadow.CANME.all = ECanaRegs.CANME.all;    //使能邮箱mailbox0
    ECanaShadow.CANME.bit.ME0 = 1;
    ECanaRegs.CANME.all = ECanaShadow.CANME.all;

    ECanaRegs.CANME.all = 0;
    ECanaMboxes.MBOX16.MSGID.all = 0x00001111;      //设置接收邮箱ID号，扩展帧
    ECanaShadow.CANMD.all = ECanaRegs.CANMD.all;
    ECanaShadow.CANMD.bit.MD16 = 1;                 //设置邮箱16为接收邮箱
    ECanaRegs.CANMD.all = ECanaShadow.CANME.all;
    ECanaMboxes.MBOX16.MSGCTRL.bit.DLC = 8;
    ECanaMboxes.MBOX16.MSGCTRL.bit.RTR = 0;
    ECanaShadow.CANME.all = ECanaRegs.CANME.all;
    ECanaShadow.CANME.bit.ME16 = 1;                 //邮箱使能
    ECanaRegs.CANME.all = ECanaShadow.CANME.all;
    EALLOW;                                         //邮箱中断使能
    ECanaRegs.CANMIM.all = 0xFFFFFFFF;
    ECanaRegs.CANMIL.all = 0;                       //邮箱中断将产生在ECAN0 INT
    ECanaRegs.CANGIF0.all = 0xFFFFFFFF;
